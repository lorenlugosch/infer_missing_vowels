...
class EncoderRNN(torch.nn.Module):
	def __init__(self, num_encoder_layers, num_encoder_hidden, input_size, dropout):
		...
		self.gru = torch.nn.GRU(input_size=Sx_size, hidden_size=num_encoder_hidden, num_layers=num_encoder_layers, batch_first=True, dropout=dropout, bidirectional=True)
...
class EncoderDecoder(torch.nn.Module):
	...
	def forward(self, x, y):
		...
		for u in range(0, U):
			...
			log_p_yu = (decoder_out * y[:,u,:]).sum() # y_u is one-hot; use dot-product to select the y_u'th output probability
